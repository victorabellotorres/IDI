...
void ExamGLWidget::modelTransformGround()
{
  ...
  TG = glm::scale(TG, glm::vec3 (escalaModels[GROUND_BRICKS], escalaModels[GROUND_BRICKS], escalaModels[GROUND_BRICKS]));
...
void ExamGLWidget::calculaCapsaModel (Model &p, float &escala, float ampladaDesitjada, glm::vec3 &centreCapsa)
{
...
{  
  ...
  {	
	  ...
	  calculaCapsaModel (models[i], escalaModels[i], ampladesDesitjades[i], centreCapsaModels[i]);
